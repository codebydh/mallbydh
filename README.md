...
- 메인 배너 하단에 두 종류의 상품 진열이 노출됩니다. (Owl Carousel 사용)
  ...
  - MD's Pick! : 관리자 페이지에서 설정한 '상품 우선순위' 에 따라 진열되며, 동순위일 경우 최근 등록된 상품이 먼저 진열


#### 로그인페이지
- 입력한 아이디 및 비밀번호로 로그인을 진행하며, 서버의 응답에 따른 절차를 수행합니다. (아래는 로그인이 거절되는 유형)
 ...
 - 회원 테이블 DB에 필요한 정보를 form으로 작성하도록 제공합니다.
   ...
   - 서버 조회를 통한 ID 중복체크를 실시합니다.
   - 이메일 인증을 수행하며, 사용자가 입력한 이메일로 인증코드를 보내 검증합니다.
   - 주소는 [DAUM 주소 API](https://postcode.map.daum.net/guide)의 iframe을 이용한 레이어 띄우기 방식을 채용했습니다.
