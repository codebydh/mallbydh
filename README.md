...
#### 리뷰관리
- 등록된 모든 리뷰를 조회할 수 있으며, 리뷰내용 또는 별점으로 검색할 수 있습니다.
- 관리자는 회원이 등록한 리뷰를 직접 수정하거나, 삭제(DELETE)할 수 있습니다.

#### 문의관리
- 등록된 모든 리뷰를 조회할 수 있으며, 문의내용 또는 답변여부로 검색할 수 있습니다.
- 관리자는 회원이 등록한 문의를 직접 수정하거나, 삭제(DELETE)할 수 있습니다.
- 관리자는 한 개의 문의에 대해 1개 이상의 답변을 등록할 수 있으며, 등록된 답변은 리스트에서 <br>
  직관적으로 조회가 가능합니다. 또한 등록한 답변을 수정하거나 삭제할 수 있습니다.

<br>

## 프로젝트 후기
#### :computer: 코드를 작성하는 것도 중요하지만, '어떻게 짤 것인가'에 대한 생각이 선행되어야 한다.
&emsp; 예시를 들면, 프로젝트 초기에는 상품 결제 기능을 단순히 결제가 가능하면 된다는 생각으로 기획하고 개발했습니다.
그러나 돌아보니 상품에는 재고 관리와 누적 판매량 같은 중요한 요소를 결제 단계에서 함께 처리해야 한다는 점을 뒤늦게 인식하고 반영하게 되었습니다.
이런 경험을 통해, 백엔드 작업에서는 표면적으로 생각할 수 있는 작업 외에도 많은 요소들이 함께 고려되어야 한다는 것을 깊이 깨달았습니다.
<br>
&emsp; 같은 차원에서, 개발하고자 하는 제품의 비즈니스 로직을 철저히 이해하는 것이 코드 작성만큼 중요하다는 생각을 하게 되었습니다.
"아는 만큼 보인다"는 말처럼, 개발자가 비즈니스 로직을 적극적으로 파악하고 이해해야만 더 정밀하고 적절한 코드를 작성할 수 있음을 이 프로젝트를 통해 직접 체감할 수 있었습니다.

#### :cloud: 돌아보니 다소 아쉬웠던 모습들
&emsp; 백엔드 프로젝트이지만, 실제로 운영하고 있는 쇼핑몰로 보이고 싶었습니다. 그래서 의도치 않게 UI 구상에 시간을 들인 감이 없지 않아 있습니다.
개인적으로 프론트엔드 개발에도 관심이 있는지라 이런 점은 오히려 환영이었지만, 반대급부로 프로젝트 기간을 적절히 조정할 수 있었으면 하는 아쉬움이 있습니다.
<br>
&emsp; '백엔드 프로젝트'로서 아쉬웠던 점을 꼽자면, DB 설계가 조금 아쉽다는 생각을 하고 있습니다.
프로젝트를 진행하면서 DB 구조를 많이 수정도 했지만, 시간이 지남에 따라 지식이 조금씩 늘면서 DB 정규화에 대해 좀 더 신경 쓸 수 있었다는 점이 아쉬웠습니다.
그런 점에서 앞으로 진행할 다른 프로젝트에서는 DB 설계에 대해 좀 더 신중하게 생각해볼 수 있게 하는 계기가 되기도 했습니다.

#### :sunny: 웹 개발에 대한 지식과 흥미를 본격적으로 돋군 계기
&emsp; 이전 경험을 통해 웹 개발이라는 것에 대한 모호한 개념은 이미 있던 상태였지만, 정작 '개발'이 뭔지 몰라 정확히는 알지 못했습니다.
하지만 백엔드 개발에 대한 학습을 하게 되고, 배운 것을 토대로 실제로 프로젝트를 수행해 가면서 웹 개발이 무엇인지에 대해 과거에 비해서 확실하게 알게 되었습니다.
<br>
&emsp; 로직을 설계하고, 로직에 맞는 화면을 그리고, 기능을 구현하여 화면에 적용하는 모든 과정이 처음에는 다소 어려웠지만, 갈 수록 어려움에서 오는 막막함보다 모르는 것에 대한 흥미로움과 파헤쳐 나갔을 때의 기쁨이 더욱 크게 쌓여나갔습니다.
할 수 있는게 많을 수록 더욱 재미있게 파헤쳐 나갈 수 있는 분야임을 알게 되었고, 앞으로도 꾸준히 학습하고 프로젝트를 수행하며 깊이 있는 개발자로 거듭나고 싶습니다.
